...
string UserModel::notify_changes()
{
  ...
  for(vector<UserTermRecord *>::iterator it = userTermRecords.begin(); it != userTermRecords.end(); ++it) {
    ...
    {
      string url = "GET https://es.wikipedia.org/w/api.php?action=query&prop=revisions&titles=" + string((*it)->entry_key) + "&rvlimit=2&rvslots=main&rvprop=timestamp%7Cuser%7Ccomment&rvdir=newer&rvstart=2018-07-01T00:00:00Z&format=json HTTP/1.1\r\nConnection: close\r\n\r\n";
